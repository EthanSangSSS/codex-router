...
Only the primary Codex task may create Agents; the first and only child Agent permitted by Router policy is one persistent `luna_worker`, and Luna or any other child Agent must not create descendants. The generated `agents/luna-worker.toml` contains `[agents]` with `enabled=false`, so Luna's multi-Agent tools are disabled. When the interface supports it, Sol creates the initial Luna from a self-contained packet with no conversation history, then reuses that same Luna for later packets, including completed or idle states. Every packet restates its packet id, working directory, allowed paths, forbidden operations, validation, stop conditions, and required output, and the previous packet's path authorization expires automatically. Capacity exhaustion does not authorize Sol takeover; reuse the existing Luna, and no recovery may depend on creating, closing, or relaying through a completed non-Luna child. Use `BLOCKED_LUNA_CAPACITY` only when visible open blockers exist. Use `BLOCKED_LUNA_RUNTIME_CAPACITY_DESYNC` only when the Agent tree has no reusable Luna, a known Luna cannot be addressed, but `spawn_agent` reports capacity full. Neither state authorizes Sol writable takeover or relay. Never archive the task as recovery. Router cannot repair Codex runtime capacity accounting. Only direct/bypass, an unsafe architectural decomposition, or a non-capacity Luna execution blocker permits a disclosed bounded Sol takeover.

The guard applies to each spawn request independently: it permits only an explicit `agent_type=luna_worker` with valid required input and fail-closed denies non-Luna or malformed input. It cannot enforce singleton state or repair Codex runtime capacity accounting; the managed AGENTS policy requires one persistent Luna and reuse.

Install only from a durable Python environment where `codex_router` is installed for the same absolute interpreter recorded in the hook commands. The `hooks.json` installation keeps one `UserPromptSubmit` command handler and adds exactly one `PreToolUse` group with matcher `^(Agent|spawn_agent)$` containing one `hook-agent-spawn` guard. Both commands use `-E -P -m codex_router` so they cannot depend on the caller's `PYTHONPATH` or working directory. Before changing managed files, installation preflights the prompt handler plus an allowed Luna spawn with empty output, a non-Luna denial, and a malformed-input denial. A failed probe leaves all managed user files unchanged:
