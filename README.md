...
K1 uses a control-plane sideband on current Multi-Agent V2. For every generation, PRIMARY constructs canonical K1 and stages it through `router stage-k1` with the one-time current-root capability before calling native `spawn_agent` or `followup_task`. The native `message` is a transport trigger, not authority: Router never parses, compares, or searches it for K1 plaintext. `send_message` remains QueueOnly; it cannot consume staged K1 or advance a generation. Sideband routing is compatible only when exact runtime evidence positively exposes `router_stage_k1_exec`; an absent or ambiguous capability remains unknown rather than being inferred from the V2 tool surface.
