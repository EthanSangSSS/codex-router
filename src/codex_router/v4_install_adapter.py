...
- For a substantive `route`, use the complete injected `K1_STAGE_COMMAND` verbatim. Write exactly one seven-field UTF-8 JSON request to the exact absolute path following `--request-file`: `packet_id`, `objective`, `working_directory`, `intended_write_scope`, `explicit_side_effect_authorizations`, `success_criteria`, and `stop_conditions`. Do not append semantic K1 fields to the shell argv and do not add generation, session, lease, capability, or agent identity to the request. Router alone constructs canonical K1.
- Successful V4 staging returns the generation-scoped `task_name`, `bootstrap_capability`, and `spawn_message`. With the native V2 spawn surface, spawn exactly the task_name returned by staging using `agent_type=luna_worker`, `fork_turns=none`, and the spawn_message returned by staging unchanged. Never substitute the static task name `luna_worker` for the returned generation-scoped task_name.
