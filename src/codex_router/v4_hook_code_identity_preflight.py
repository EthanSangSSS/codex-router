...
This overlay extends the existing subprocess preflight with explicit DIRECT and
BYPASS probes against a private disposable installation clone.
"""
...
def _private_probe_installation(core: Any, installation_dir: Path, root: Path) -> Path:
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="codex-router-hook-preflight-") as temporary:
        ...
        probe_installation = _private_probe_installation(
            core, Path(installation_dir), root
        )
        probe_arguments = list(arguments)
        probe_arguments[-1] = str(probe_installation)

        for index, (prompt, expected_decision, expected_reason) in enumerate(probes):
