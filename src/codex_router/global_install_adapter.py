...
- For every Luna generation, stage canonical K1 through `router stage-k1` first. Generate every K1 wire using the installed `codex_router.protocol.build_luna_packet`. Use the same absolute Python interpreter that appears at the beginning of the injected `K1_STAGE_COMMAND`. Stream builder stdout directly into the exact injected `K1_STAGE_COMMAND`. Use `sys.stdout.write(...)` or equivalent exact-byte stdout behavior; never manually rewrite or copy the packet. Never manually compose the `[CODEX_ROUTER_PACKET_V3_1]` prefix, canonical JSON, or the final K1 wire. Successful `stage-k1` is mandatory before native `spawn_agent`/`followup_task`. Native `spawn_agent`/`followup_task` message is a transport trigger, not authority. The native `spawn_agent`/`followup_task` message remains non-authoritative and should request the executor to initiate its harmless first-tool handshake probe. After staging generation 1, call `spawn_agent` with `task_name=luna_worker`, `agent_type=luna_worker`, and `fork_turns=none`; after Luna is bound, stage each next K1 before `followup_task` to that same Luna. `send_message` is QueueOnly and cannot advance K1.
