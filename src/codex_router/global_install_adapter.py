...
- For every Luna generation, stage canonical K1 through `router stage-k1` first. Native `spawn_agent`/`followup_task` message is a transport trigger, not authority. After staging generation 1, call `spawn_agent` with `task_name=luna_worker`, `agent_type=luna_worker`, and `fork_turns=none`; after Luna is bound, stage each next K1 before `followup_task` to that same Luna. `send_message` is QueueOnly and cannot advance K1.
...
- Native collaboration messages are transport triggers, not work authority. The authoritative work packet is `[CODEX_ROUTER_PACKET_V3_1]` injected by Router as developer context. Do not perform tool work for a new generation until Router performs the first-tool authority handshake.
