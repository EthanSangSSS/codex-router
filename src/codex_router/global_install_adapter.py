...
- For the first Luna generation, build the generation-1 K1 packet before spawning. Call `spawn_agent` with `task_name=luna_worker`, `agent_type=luna_worker`, `fork_turns=none`, and the exact generation-1 K1 packet as `message`; never spawn Luna with placeholder or non-K1 work. After that Luna is bound, later generations use `followup_task` to the same Luna with the next complete K1 packet. Exact current-App `send_message` is queue-only and must not admit a new K1 generation.
