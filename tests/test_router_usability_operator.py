...
class RouterUsabilityV33OperatorTests(unittest.TestCase):
    _V1_NO_FOLLOWUP = {
    ...
    _NO_SPAWN = {
        ...
        "spawn_agent": False,
        "multi_agent_v1__spawn_agent": False,
        "followup_task": False,
    }
    ...
    def test_followup_unavailable_does_not_block_fresh_generation_spawn(self) -> None:
        decision = adapter.primary_gen2_readiness(self._V1_NO_FOLLOWUP)

        self.assertEqual(decision["code"], "READY")
        self.assertEqual(decision["reason_code"], "NATIVE_SURFACE_COMPATIBLE")
        self.assertEqual(decision["persistent_followup_availability"], "UNAVAILABLE")

    def test_spawn_unavailable_degrades_only_when_safe_and_non_strict(self) -> None:
    ...
    def test_one_argument_readiness_uses_fresh_spawn_not_followup(self) -> None:
        legacy_name = adapter.primary_gen2_readiness(self._V1_NO_FOLLOWUP)
        self.assertEqual(legacy_name["code"], "READY")

    def test_policy_text_treats_absent_strict_flag_as_non_strict(self) -> None:
        text = adapter.AGENTS_BLOCK_V3
        self.assertIn("`strict_router` is not true", text)
        self.assertIn("mechanically reports", text)
        self.assertIn("SAFE_LOCAL_FALLBACK", text)

    def test_v33_luna_instructions_do_not_contain_denial_retry_protocol(self) -> None:
        text = adapter.LUNA_DEVELOPER_INSTRUCTIONS_V3
        self.assertIn("harmless first-tool handshake probe", text)
